...
//go along the built huffman tree and assign the code for each character
...
// to read the file appropraitely
void import_file(FILE *fp_in, unsigned int *freq){
	...
	while (qend > 2) qinsert(new_node(0, 0, qremove(), qremove()));	//build the tree
...
int main(int argc, char* argv[]){
	FILE *fp_in, *fp_out;			     //File in and File out pointers
